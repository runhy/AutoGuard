...
def agent_classify(text: str, uploaded_file=None) -> tuple[str, str, bool]:
    """
    [1차 에이전트] 입력값을 분석해 유형 분류 + 1차 악성 판단을 수행한다.
    이메일 본문 + URL / 이메일 본문 + 파일 등 복합 입력도 지원한다.

    반환: (analysis_type, reason_message, needs_api)
    - analysis_type : URL | Email | File | Email+URL | Email+File | Email+URL+File | Unknown
    - reason_message: UI에 표시할 감지 근거
    - needs_api     : True = 2차 API 배분 필요 / False = 1차에서 분류 완료
    """
...
def agent_dispatch_to_api(text: str) -> tuple[str, str]:
    """
    [2차] 1차 에이전트가 분류하지 못한 입력을 키워드 기반으로
    URL / Email / File API 중 하나로 배분한다.
    """
...
<style>
    ...
    /* Streamlit 파일 업로더 "Limit 200MB" 기본 문구 숨기기 */
