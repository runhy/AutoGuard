...
<style>
    ...
    .history-content { 
        ...
        margin-top: -3px !important; /* 글자 덩어리를 위로 살짝 당김 */
    }
    /* 스트림릿이 몰래 넣는 위아래 마진 완벽 제거 */
    .history-content p {
        margin: 0 !important;
        padding: 0 !important;
    }
    ...
    section[data-testid="stSidebar"] [data-testid="stHorizontalBlock"]:has(.history-high) {
        border-left: 5px solid #d32f2f !important; /* 왼쪽 강조 선 살짝 더 두껍게 */
    }
    section[data-testid="stSidebar"] [data-testid="stHorizontalBlock"]:has(.history-medium) {
        border-left: 5px solid #f57c00 !important;
    }
    ...
    /* X 삭제 버튼 스타일 - 불필요한 테두리 없애고 깔끔하게 */
    section[data-testid="stSidebar"] [data-testid="stHorizontalBlock"]:has(.history-content) button {
        background: transparent !important;
        border: none !important; /* 👈 답답해 보이는 회색 네모 테두리 완전 제거 */
        box-shadow: none !important;
        color: #999 !important;
        font-size: 1.1rem !important; /* X 표시 조금 더 크게 */
        padding: 8px !important;
        min-height: unset !important;
        line-height: 1 !important;
        border-radius: 6px !important;
    ...
    section[data-testid="stSidebar"] [data-testid="stHorizontalBlock"]:has(.history-content) button:hover {
        color: #d32f2f !important;
        background: rgba(211, 47, 47, 0.1) !important; /* 마우스 올렸을 때 연한 빨간 배경 */
    }
   ...
   /* 텍스트 입력창: 입체감 뿜뿜 (그림자 2배 + 왼쪽 파란색 포인트 띠) */
    div[data-testid="stTextArea"] textarea {
        background-color: #ffffff !important;
        border: 1px solid #cbd5e1 !important;
        border-left: 5px solid #3b82f6 !important; /* 왼쪽 파란색 포인트 확 강조! */
        border-radius: 8px !important;
        box-shadow: 0 4px 6px -1px rgba(0,0,0,0.15), 0 2px 4px -1px rgba(0,0,0,0.1) !important; /* 그림자 깊게 */
    }
    
    /* 파일 업로드 영역: 옅은 푸른/회색 배경과 진한 점선으로 '업로드 구역' 느낌 팍팍 */
    div[data-testid="stFileUploader"] > section {
        background-color: #f1f5f9 !important; 
        border: 2px dashed #64748b !important; /* 점선을 훨씬 굵고 진하게 */
        border-radius: 8px !important;
        padding: 1.5rem !important; /* 안쪽 여백을 늘려서 큼직하게 */
    }
    /* --- 메인 '분석 시작' 버튼 파란색(#1D4ED8)으로 변경 --- */
    div.stButton > button[kind="primary"] {
        background-color: #1D4ED8 !important;
        border-color: #1D4ED8 !important;
        color: #ffffff !important;
    }
    /* 마우스를 올렸을 때 살짝 어두운 파란색으로 변하는 효과 */
...
if st.button("🔍 분석 시작", type="primary", use_container_width=True):
    ...
    if uploaded_target_file is None and not user_input.strip():
        st.warning("URL, 이메일, 또는 파일을 입력해 주세요.")
        st.stop()
...
with st.sidebar:
    ...
    # 배지를 더 크고 입체적으로 (비활성은 빨간색 계열로 눈에 확 띄게)
    def get_badge(is_active):
        if is_active:
            bg_color = "#dcfce7" # 확 띄는 연두색 바탕
            text_color = "#166534" # 진초록 글씨
            border = "1px solid #bbf7d0"
            icon = "✅ 활성"
        else:
            bg_color = "#fee2e2" # 눈에 확 띄는 연한 빨간색 바탕
            text_color = "#b91c1c" # 붉은색 글씨
            border = "1px solid #fecaca"
            icon = "❌ 비활성"
            
        return f'<span style="background:{bg_color}; color:{text_color}; border:{border}; padding:4px 12px; border-radius:14px; font-size:0.75rem; font-weight:800; margin-left:8px; box-shadow:0 1px 2px rgba(0,0,0,0.1);">{icon}</span>'
...
components.html(
    ...
    function bindEnterKey() {
        ...
        textareas.forEach(ta => {
            if (!ta.hasAttribute('data-enter-bound')) {
                ...
                ta.addEventListener('keydown', function(e) {
                    if (e.key === 'Enter' && !e.shiftKey) {
                        ...
                        setTimeout(() => {
                            ...
                            buttons.forEach(btn => {
                                if (btn.innerText.includes('분석 시작')) {
                                    btn.click();
                                }
                            });
                        }, 200); 
