...
def mock_fastapi_request(_user_input: str = "", analysis_type: str = "URL") -> dict:
    ...
    mock_profiles = {
        ...
        "Email": {
            ...
            "vt_result": {
                "detection_ratio": "8/90",
                "malicious_count": 8,
                "harmless_count":  82,
            },
            "ai_agent_report": {
                "summary": (
                    "이 이메일은 사회공학(Social Engineering) 기법을 활용한 "
                    "스피어 피싱 공격으로 판단됩니다. "
...
if st.button("🔍 분석 시작", type="primary", use_container_width=True):
    ...
    if analysis_type != "File":
        ...
        if detected and detected != analysis_type:
            type_names = {"URL": "URL", "Email": "이메일"}
            st.info(
                f"💡 입력값이 **{type_names.get(detected, detected)}** 형식으로 감지되어 "
                f"분석 유형을 **{detected}**(으)로 자동 전환합니다."
            )
