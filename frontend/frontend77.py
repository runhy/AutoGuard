...
with col_left:
    ...
    input_container = st.empty() # 👈 나중에 갈아끼울 수 있도록 빈 공간(컨테이너) 확보

    user_input = input_container.text_area(
        "텍스트 입력",
        key=f"text_input_{st.session_state.input_key}", # 👈 고유 키 부여
        placeholder=(
            "예시:\n"
...
if st.button("🔍 분석 시작", type="primary", use_container_width=True):
    ...
    current_input = st.session_state.get("main_input", "").strip()
    ...
    # 👇 분석 완료 후 텍스트 입력창 완벽 초기화 
    st.session_state.input_key += 1
    input_container.text_area(
        "텍스트 입력",
        value="", 
        key=f"text_input_{st.session_state.input_key}",
        placeholder="분석 완료! 새로운 분석 대상을 입력하세요...",
        height=140,
        label_visibility="collapsed",
    )
...
// 화면이 수백 번 갱신되어도 가장 최상위 창에 딱 한 번만 영구적으로 감지기를 등록합니다.
if (!topWin._ag_key_listener_installed) {
    topWin._ag_key_listener_installed = true;
    
    // true(캡처링) 옵션으로 스트림릿보다 무조건 먼저 키보드 입력을 낚아챕니다.
    topWin.addEventListener('keydown', function(e) {
        if (e.target && e.target.tagName === 'TEXTAREA') {
            ...
            if (e.key === 'Enter' && !e.shiftKey) {
                ...
                const ta = e.target;
                const btns = Array.from(topWin.document.querySelectorAll('button[kind="primary"]'));
                const submitBtn = btns.find(b => b.innerText.includes('분석 시작'));
                
                if (submitBtn) {
                    ta.blur(); // 커서를 빼서 파이썬으로 현재 글자 확정 전송
                    setTimeout(() => { submitBtn.click(); }, 150); // 버튼 클릭!
                }
            }
        }
    }, true);
